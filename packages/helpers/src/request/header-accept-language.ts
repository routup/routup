...
export function getRequestAcceptableLanguages(req: IncomingMessage) : string[] {
    const negotiator = useRequestNegotiator(req);
    return negotiator.languages();
}

export function getRequestAcceptableLanguage(req: IncomingMessage, input?: string | string[]) : string | undefined {
    input = input || [];
